...
import {HorizontalFoodCard, VerticalFoodCard} from 'components/common';
...
const HomeScreen = () => {
  ...
  const handleChangeCategory = (categoryId, menuTypeId) => {
    // recommended menu
    let selectedRecommended = data.menu.find(a => a.name == 'Recommended');
    ...
    let selectedMenu = data.menu.find(a => a.id == menuTypeId);
        ...
        <TouchableOpacity
          style={[
            styles.foodCategoriesBtn,
            {
              marginLeft: index == 0 ? SIZES.padding : 18,
              marginRight:
                index == data.categories.length - 1 ? SIZES.padding : 0,
              backgroundColor:
